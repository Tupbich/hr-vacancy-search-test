                    ...
                    <template v-if="mode=='metro'">
                        <q-item-section>
                            <q-item-label :class="{'text-accent': containsInInput(s.text)}">
                                <q-icon :name="modeIcon" size="22px"
                                        :style="{'color': '#'+s.obj.HexColor}" />
                                линия {{ s.text }}
                            </q-item-label>
                            <q-item-label caption v-if="s.childs">
                                станции:
                                <q-chip v-for="ss in s.childs" :key="ss.text" class="metro-station"
                                        :color="containsInInput(ss.text)?'accent':'primary'"
                                        text-color="white" dense outline clickable
                                        @click.stop="onSuggestionClick(ss)">
                                    {{ss.text}}</q-chip>
                            </q-item-label>
                        </q-item-section>
...
<script lang="ts" src="./LocationSearch.ts" />
